...
import type { CommandDefinitionLike, CommandInvocationLike, CommandResultLike } from '../host-types.js'
...
const PREFIX_OPERATORS: readonly string[] = ['|', ';', '&', '`', '$']

/** 构造成功结果. */
function success(text: string): CommandResultLike {
  return { kind: 'success', text }
}

/** 构造失败结果. */
function failure(text: string): CommandResultLike {
  return { kind: 'error', text }
}

/** 把条目渲染成带序号的缩进行, 空表给出占位. */
function renderEntries(entries: readonly string[]): string[] {
  if (entries.length === 0) return [EMPTY_SECTION]
  return entries.map((entry, index) => `  ${String(index + 1)}. ${entry}`)
}

/** 渲染 "工具: 前缀" 形式的条目. */
function renderToolEntries(entries: readonly { readonly tool: string; readonly prefix: string }[]): string[] {
 ...
 * @returns 该段的所有行.
 */
function renderSessionSection(context: ApprovePrefixCommandContext, sessionKey: string | undefined): string[] {
 ...
 * @returns 直接展示给用户的文本.
 */
function renderStatus(context: ApprovePrefixCommandContext, sessionKey: string | undefined): string {
  return [
    '放行前缀',
    '',
    STATIC_SECTION,
...
/** 结果行加当前会话段. */
function withSessionSection(head: string, context: ApprovePrefixCommandContext, sessionKey: string): string {
  return [head, '', ...renderSessionSection(context, sessionKey)].join('\n')
}

/** 参数缺失时的两行提示. */
function argumentHint(command: string): string {
  return [
...
function parsePrefixArgument(rest: string, defaultTool: string | undefined): { tool: string; prefix: string } | undefined {
  if (rest === '' || defaultTool === undefined) return undefined
...
function isFailure<T>(value: T | CommandResultLike): value is CommandResultLike {
  return typeof value === 'object' && value !== null && 'kind' in value
}

/** 检查前缀里的 shell 运算符. */
function prefixRejection(prefix: string): string | undefined {
  for (const character of PREFIX_OPERATORS) {
    if (prefix.includes(character)) return `前缀里不能出现 shell 运算符 "${character}"`
  }
  return undefined
}
 ...
 */
export function createApprovePrefixCommands(context: ApprovePrefixCommandContext): CommandDefinitionLike[] {
  /** 读取前缀参数并做公共校验; 出错时返回错误结果. */
  const readPrefixArgument = (
    invocation: CommandInvocationLike,
    command: string,
  ): { tool: string; prefix: string } | CommandResultLike => {
    const parsed = parsePrefixArgument(invocation.rawInput.trim(), context.tools[0])
    if (parsed === undefined) return failure(argumentHint(command))
  ...
  /** 不接受参数的命令共用的守卫. */
  const noArgument = (invocation: CommandInvocationLike, command: string): CommandResultLike | undefined => {
  ...
  const sessionOrError = (
    invocation: CommandInvocationLike,
    command: string,
  ): { sessionKey: string } | CommandResultLike => {
    const sessionKey = sessionKeyOf(invocation.agent)
    if (sessionKey === undefined) return failure(`/${command} 需要会话身份, 但这次调用没有带 agent`)
    return { sessionKey }
  }
  ...
  return [
    {
      ...
      handler: (invocation) => {
        ...
        return success(withSessionSection(
          `已加入本次会话: ${parsed.tool}: ${parsed.prefix}`,
          context,
          session.sessionKey,
        ))
      },
    },
    {
      ...
      handler: (invocation) => {
        ...
        return success(withSessionSection(
          `已从本次会话移除: ${parsed.tool}: ${parsed.prefix}`,
          context,
          session.sessionKey,
        ))
      },
