 ...
 */
export const STRUCTURAL_METACHARACTERS = ['|', '&', ';', '<', '>', '`', '$', '\n', '\r'];
 ...
 * @returns 检查结果.
 */
export function inspectSingleCommand(command, extraDeniedCharacters = []) {
    const text = command.trim();
    if (text === '')
        return { ok: false, tokens: [], strippedEnvironment: false, detail: 'the command is empty' };
 ...
 * @returns 判定结果.
 */
export function judgeSingleCommandPrefix(command, prefixes, extraDeniedCharacters = []) {
