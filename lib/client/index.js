...
function decodeSettings(section) {
    ...
    const raw = section[PERSISTENT_PREFIXES_FIELD];
    if (!Array.isArray(raw))
        return { persistentPrefixes: [] };
    const persistentPrefixes = raw.flatMap((entry) => {
 ...
 */
function validateEntry(entry, translate) {
    if (entry.tool.trim() === '')
        return translate('error.toolEmpty');
    if (entry.prefix.trim() === '')
        return translate('error.prefixEmpty');
    for (const character of ['|', ';', '&', '`', '$']) {
        if (entry.prefix.includes(character))
            return `${translate('error.operator')}: "${character}"`;
    }
    return undefined;
}
/** 组装 client 半边. */
    ...
    return {
        ...
        apply(ctx) {
            ...
            const Page = () => {
                ...
                const [draft, setDraft] = React.useState(readStored);
                ...
                const rows = draft.map((entry, index) => React.createElement('div', { className: 'field', key: `row-${String(index)}` }, React.createElement('input', {
                    className: 'tool',
                    value: entry.tool,
                ...
                }), React.createElement('button', { onClick: () => setDraft(draft.filter((_entry, position) => position !== index)) }, translate('action.remove'))));
                const cardChildren = [
                    React.createElement('div', { className: 'head', key: 'head' }, React.createElement('span', { className: 'tool' }, translate('column.tool')), React.createElement('span', { className: 'prefix' }, translate('column.prefix')), React.createElement('span', { className: 'tail' }, '')),
                ];
                if (rows.length === 0)
                    cardChildren.push(React.createElement('div', { className: 'empty', key: 'empty' }, translate('empty')));
                else
                    cardChildren.push(...rows);
                cardChildren.push(React.createElement('div', { className: 'actions', key: 'actions' }, React.createElement('button', { onClick: () => setDraft([...draft, { tool: 'bash', prefix: '' }]) }, translate('action.add')), React.createElement('button', { className: 'primary', onClick: save }, translate('action.save')), React.createElement('button', {
                    onClick: () => {
                        setDraft(stored.map(entry => ({ ...entry })));
                        setStatus({ text: translate('status.reloaded'), error: false });
                    },
                }, translate('action.reload')), React.createElement('span', { className: status.error ? 'status error' : 'status' }, status.text)));
                return React.createElement('section', { 'data-dsh-approve-prefix': '' }, React.createElement('h2', null, translate('title')), React.createElement('p', { className: 'intro' }, translate('intro')), React.createElement('div', { className: 'card' }, ...cardChildren));
            };
